---
layout: default
title: 
---

<style>
  /* Page background that matches the outer tones of the image */
  body {
    background-color: #e4e1dc; /* Sampled from image edge */
    margin: 0;
    padding: 0;
  }

  .quote-section {
    padding: 60px 15px;
  }

  .quote-container {
    position: relative;
    max-width: 700px;
    margin: 0 auto;
    border-radius: 8px;
    overflow: hidden;
  }

  .background-image {
    width: 100%;
    height: auto;
    display: block;
  }

  /* Quote text at top right */
  .quote-text {
    position: absolute;
    top: 20px;
    right: 20px;
    max-width: 300px;
    font-size: 16px;
    line-height: 23px;
    font-weight: 400;
    color: #000000;
    padding: 0;
    margin: 0;
    margin-bottom: 15px;
    font-family: Georgia, serif;
    text-align: right;
  }

  /* Artist text at bottom left */
  .artist-overlay {
    position: absolute;
    bottom: 20px;
    left: 20px;
    font-family: Arial, sans-serif;
    font-style: italic;
    font-size: 1rem;
    color: #000000;
    padding: 0;
    margin: 0;
  }

  .artist-overlay a {
    color: inherit;
    text-decoration: underline;
    font-weight: normal;
  }

  /* Responsive tweaks */
  @media (max-width: 768px) {
    .quote-text {
      font-size: 14px;
      line-height: 20px;
      top: 15px;
      right: 15px;
      max-width: 90%;
    }

    .artist-overlay {
      font-size: 0.9rem;
      bottom: 15px;
      left: 15px;
    }
  }

  @media (max-width: 480px) {
    .quote-text,
    .artist-overlay {
      font-size: 14px;
      line-height: 20px;
      max-width: 90%;
    }

    .quote-text {
      top: 10px;
      right: 10px;
    }

    .artist-overlay {
      bottom: 10px;
      left: 10px;
    }
  }
</style>

<div class="quote-section">
  <div class="quote-container">
    <img 
      src="https://www.densediscovery.com/archive/354/head.jpg" 
      alt="Artwork by Janusz Jurek" 
      class="background-image" 
    />
    <p class="quote-text">
      Designs who engage in ‘the design of dissent’ do exist, but design’s default position is to grease the wheels of capitalism with style and taste.<br><br>
      – Adapted from Jeffrey Keedy
    </p>
    <div class="artist-overlay">
      Featured artist: <a href="https://www.instagram.com/januszjurek.info/" target="_blank" rel="noopener noreferrer">Janusz Jurek</a>
    </div>
  </div>
</div>
